...
- `codex/fix-notification-self-contained-runtime`
- `codex/cleanup-integration-notification-runtime`
...
git merge --no-ff codex/fix-notification-self-contained-runtime
git merge --no-ff codex/cleanup-integration-notification-runtime
...
The project publish target resolves the notification resource from the restored `Microsoft.WindowsAppSDK.Runtime` NuGet package, so it stays aligned with the package version and architecture used by the project. The helper validates the executable, PRI, loose XBF resources, and the notification resource's valid Microsoft Authenticode signature before replacing the current build. It keeps the prior build at:
