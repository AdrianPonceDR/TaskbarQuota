...
- **Current-session comparison** — by default, the first live reading after launch or re-enabling the option establishes a baseline without notifying.
- **Changes since last session** — a separate, off-by-default option can compare the first live reading after startup with the last confirmed live observation saved by TaskbarQuota. It can report a replenishment that happened while the app or PC was off, but the notification is retrospective and only appears once TaskbarQuota runs and refreshes that provider.
- **Conservative startup matching** — cross-session comparison requires the same provider, a matching one-way account identity hash, stable window key, title, duration, and a saved observation no more than 35 days old. Missing or ambiguous identity, stale state, cache, restored UI snapshots, failures, and changed window definitions establish a new baseline silently.
- **Local cross-session state** — `quota-replenishment-state.json` contains only percentage-window metadata, timestamps, and a one-way identity hash, never the plaintext account identifier. Disabling either replenishment notifications or the cross-session option clears this state.
